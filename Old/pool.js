...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function OutputPoolInfo() {
        ...
        var infoLines = [startMessage,
            ...
            'Pool PoolHex ID:\t' + _this.options.poolHex
    ...
    function OnBlockchainSynced(syncedCallback) {
        ...
        checkSynced(function () {
            //Only let the first fork show synced status or the log wil look flooded with it
            if (!process.env.forkId || process.env.forkId === '0')
                emitErrorLog(options.coin.name + ' - Daemon is still syncing with network (download blockchain) - server will be started once synced');
        });
        ...
        var generateProgress = function () {
            ...
            _this.daemon.cmd('getinfo', [], function (results) {
                ...
                _this.daemon.cmd('getpeerinfo', [], function (results) {
                    ...
                    emitWarningLog(options.coin.name + ' - Downloaded ' + percent + '% of blockchain from ' + peers.length + ' peers');
                });
    ...
    function SetupPeer() {
        ...
        if (options.testnet && !options.coin.peerMagicTestnet) {
            emitErrorLog(options.coin.name + ' - p2p cannot be enabled in testnet without peerMagicTestnet set in coin configuration');
            return;
        }
        else if (!options.coin.peerMagic) {
            emitErrorLog(options.coin.name + ' - p2p cannot be enabled without peerMagic set in coin configuration');
            return;
        }

        _this.peer = new peer(options);
        _this.peer.on('connected', function () {
            emitLog(options.coin.name + ' - p2p connection successful');
        }).on('connectionRejected', function () {
            emitErrorLog(options.coin.name + ' - p2p connection failed - likely incorrect p2p magic value');
        }).on('disconnected', function () {
            emitWarningLog(options.coin.name + ' - p2p peer node disconnected - attempting reconnection...');
        }).on('connectionFailed', function (e) {
            emitErrorLog(options.coin.name + ' - p2p connection failed - likely incorrect host or port');
        }).on('socketError', function (e) {
            emitErrorLog(options.coin.name + ' - p2p had a socket error ' + JSON.stringify(e));
        }).on('error', function (msg) {
            emitWarningLog(options.coin.name + ' - p2p had an error ' + msg);
    ...
    function SetupRecipients() {
        ...
        if (recipients.length === 0) {
            emitErrorLog(options.coin.name + ' - No rewardRecipients have been setup which means no fees will be taken');
        }
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    emitWarningLog(options.coin.name + ' - Warning, ignored duplicate submit block ' + blockHex);
                } else {
                    ...
                    SubmitBlock(blockHex, function () {
                        CheckBlockAccepted(shareData.blockHash, function (isAccepted, tx) {
                            ...
                            GetBlockTemplate(function (error, result, foundNewBlock) {
                                if (foundNewBlock) {
                                    emitLog(options.coin.name + ' - Block notification via RPC after block submission');
                                }
                            });
                        });
                    });
            ...
            if (options.coin.disablecb) {
              if (typeof(rpcResults.getinfo.pubkey) === 'undefined') {
                  if ( typeof(rpcResults.validateaddress.pubkey) === 'undefined' ) {
                    emitErrorLog(options.coin.name + ' - pubkey is not set, and pool address is not imported. You must either import the private key or set the daemons -pubkey when using disable coinbase mode.');
                    return;
                  } else {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            emitLog(options.coin.name + ' - No new blocks for ' + options.jobRebroadcastTimeout + ' seconds - updating transactions & rebroadcasting work');
    ...
    function SetupBlockPolling() {
        if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
            emitLog(options.coin.name + ' - Block template polling has been disabled');
            return;
        }
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function (error, result, foundNewBlock) {
                if (foundNewBlock)
                    emitLog(options.coin.name + ' - Block notification via RPC polling');
            });
        }, pollingInterval);
    }
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        emitLog(options.coin.name + ' - Block notification via ' + sourceTrigger);
